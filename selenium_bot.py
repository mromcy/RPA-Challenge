...
for index, row in file.iterrows():
    ...
    ).send_keys(str(row['First Name']))
    driver.find_element(
        By.XPATH, "//label[text()='Last Name']/following-sibling::input"
    ).send_keys(str(row['Last Name']))
    driver.find_element(
        By.XPATH, "//label[text()='Company Name']/following-sibling::input"
    ).send_keys(str(row['Company Name']))
    driver.find_element(
        By.XPATH, "//label[text()='Role in Company']/following-sibling::input"
    ).send_keys(str(row['Role in Company']))
    driver.find_element(
        By.XPATH, "//label[text()='Address']/following-sibling::input"
    ).send_keys(str(row['Address']))
    driver.find_element(
        By.XPATH, "//label[text()='Email']/following-sibling::input"
    ).send_keys(str(row['Email']))
    driver.find_element(
        By.XPATH, "//label[text()='Phone Number']/following-sibling::input"
    ).send_keys(str(row['Phone Number']))
